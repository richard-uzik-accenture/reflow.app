...
- [src/hooks/useLongPressDrag.ts:33-34](../../../src/hooks/useLongPressDrag.ts#L33-L34): `e.preventDefault()` on touch `onPointerDown` may suppress the subsequent synthetic click on some browsers for a short press — likely already covered by `docs/plans/reflow-v3/archive/testing-guide.md`, worth a manual QA pass.
...
Full visual report with before/after diagrams: **[architecture-review.html](architecture-review/architecture-review.html)**.
...
- [ ] Manual QA: run through `docs/plans/reflow-v3/archive/testing-guide.md`, paying attention to the two minor notes above (touch click suppression, blur-on-mouse-drag-release).
